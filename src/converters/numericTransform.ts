...
export const numericTransform: NumericTransform = (args) => {
  const { splitFileList, customText, textPosition, preserveOriginal, numericTransform, separator, truncate } =
    args;
  ...
  return splitFileList.map((splitFile, index) => {
    ...
    const rename = composeRenameString({baseName, ext, newName:stringifiedNum, preserveOriginal, customText, textPosition, separator, truncate});
