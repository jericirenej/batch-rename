...
const { generateSearchAndReplaceArgs, searchAndReplace, omitTransform, keepTransform } =
  regexTransform;
const spyOnGenerateArguments = jest.spyOn(regexTransform, "generateSearchAndReplaceArgs");
...
describe("searchAndReplace", () => {
  ...
  it("Should call generateArguments", () => {
    ...
    expect(spyOnGenerateArguments).toHaveBeenCalledWith(exampleArgs.searchAndReplace);
  });
  ...
  it("Should preserve extension, except if noPreserveExtension is specified", () => {
    const customMocksList = generateMockSplitFileList(2).map((fileInfo, index) => ({
      ...fileInfo,
      baseName: `exterior${index + 1}`,
    }));
    const newArgs: SearchAndReplaceArgs = {
      ...exampleArgs,
...
describe("keepTransform and omitTransform", () => {
  ...
  it("Omit: With only 'omit' arg, should contain string with extension, without matchedParts", () => {
    const expected: BaseRenameList = splitFileList.map(({ baseName, ext }, index) => ({
